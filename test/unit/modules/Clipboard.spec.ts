...
import { TabulatorOptions, ColumnDefinition } from '../../../src/js/core/types';
...
describe('Clipboard', function(){
	...
	test('setPasteParser accepts function parameter', function(){
		const customParser = function(clipboard: any){
	...
	test('setPasteAction accepts function parameter', function(){
		const customAction = function(rows: any){
			return rows;
		};
	...
	test('mutateData transforms row data', function(){
		...
		expect((table.modules as any).mutator.transformRow).toHaveBeenCalledWith(
			{id: 1, name: "Test"}, 
			"clipboard"
		);
