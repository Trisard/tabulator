...
export default class SelectRow extends Module {
	...
	static moduleName: string = "selectRow";
	...
	constructor(table: TabulatorType){
		...
		this.registerTableOption("selectableRowsCheck", function(data: any, row: any){return true;}); //check whether row is selectable
	...
	initialize(): void {
		...
		if(this.table.options.selectableRows === "highlight" && (this.table.options as any).selectableRange){
			...
			if(self.table.options.selectableRows && self.table.options.selectableRows != "highlight"){
				if(self.table.options.selectableRowsRangeMode === "click"){
					...
					element.addEventListener("click", function(e: MouseEvent){
						if(!self.table.modExists("edit") || !(self.table.modules.edit as any).getCurrentCell()){
							(self.table as any)._clearSelection();
						}
					...
					element.addEventListener("mouseout", function(e: MouseEvent){
						if(self.selecting){
							(self.table as any)._clearSelection();
	...
	handleComplexRowClick(row: any, e: MouseEvent): void {
		if(e.shiftKey){
			(this.table as any)._clearSelection();
	...
	checkRowSelectability(row: any): boolean {
		if(row && row.type === "row"){
			return (this.table.options.selectableRowsCheck as (...args: any[]) => any).call(this.table, row.getComponent());
		}
	...
	_selectRow(rowInfo: any, silent?: boolean, force?: boolean): any {
		...
		if(!isNaN(this.table.options.selectableRows as any) && this.table.options.selectableRows !== true && !force){
			if(typeof this.table.options.selectableRows === "number" && this.selectedRows.length >= this.table.options.selectableRows){
		...
		if(row){
			if(this.selectedRows.indexOf(row) == -1){
				...
				if((this.table.options as any).dataTreeSelectPropagate){
	...
	_rowSelectionChanged(silent?: boolean, selected: any = [], deselected: any = []): void {
		...
		if(!silent){
			...
			selected = selected.map((row: any) => row.getComponent());

			if(!Array.isArray(deselected)){
				deselected = [deselected];
			}

			deselected = deselected.map((row: any) => row.getComponent());
	...
	childRowSelection(row: any, select: boolean): void {
		var children = (this.table.modules.dataTree as any).getChildren(row, true, true);
