...
export default class MoveColumns extends Module {
	...
	initializeColumn(column: any): void {
		...
		if(!column.modules.frozen && !column.isGroup && !column.isRowHeader){
			...
			config.mousemove = function(this: MoveColumns, e: MouseEvent){
				if(column.parent === self.moving.parent){
					if((((self.touchMove ? (e as any).touches[0].pageX : e.pageX) - Helpers.elOffset(colEl).left) + self.table.columnManager.contentsElement.scrollLeft) > (column.getWidth() / 2)){
	...
	bindTouchEvents(column: any){
		var colEl = column.getElement(),
		startXMove: any = false, //shifting center position of the cell
		nextCol: any, prevCol: any, nextColWidth: number, prevColWidth: number, nextColWidthLast: number, prevColWidthLast: number;

		colEl.addEventListener("touchstart", (e: TouchEvent) => {
		...
		colEl.addEventListener("touchmove", (e: TouchEvent) => {
			var diff, moveToCol;
		...
		colEl.addEventListener("touchend", (e: TouchEvent) => {
	...
	_bindMouseMove(){
		this.table.columnManager.columnsByIndex.forEach(function(column: any){
	...
	moveColumn(column: any, after: boolean){
		...
		if(after){
			column.getCells().forEach(function(cell: any, i: number){
