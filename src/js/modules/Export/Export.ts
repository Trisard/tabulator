...
export default class Export extends Module{
	...
	static moduleName: string = "export";

	static columnLookups: Record<string, any> = columnLookups;
	static rowLookups: Record<string, any> = rowLookups;
	...
	generateExportList(config: any, style: boolean, range: any, colVisProp: string): ExportRow[] {
		var headers, body, columns: any[] | undefined, colLookup;
	...
	rowLookup(range: any): any[] {
		...
		if(typeof range == "function"){
			range.call(this.table).forEach((row: any) =>{
	...
	headersToExportRows(columns: any[]): ExportRow[] {
		...
		function parseColumnGroup(column: any, level: number){
			...
			if(column.subGroups){
				column.subGroups.forEach(function(subGroup: any){
	...
	generateTableElement(list: ExportRow[]): HTMLElement {
		...
		rowFormatter = (this.table.options as any)["rowFormatter" + (this.colVisPropAttach)],
		setup: any = {};
		...
		setup.groupHeader = (this.table.options as any)["groupHeader" + (this.colVisPropAttach)];
	...
	generateRowElement(row: ExportRow, setup: any, styles: any): HTMLElement {
		...
		row.columns.forEach((col, i) => {
			if(col){
				...
				value = col.value,
				cellStyle: any, styleProps: string[];
	...
	mapElementStyles(from: HTMLElement, to: HTMLElement, props: string[]): void {
		if(this.cloneTableStyle && from && to){
			...
			var lookup: Record<string, string> = {
			...
			if(window.getComputedStyle){
				...
				props.forEach(function(prop){
					if(!to.style[lookup[prop] as any]){
						(to.style as any)[lookup[prop] as any] = fromStyle.getPropertyValue(prop);
					}
				});
			}
		}
