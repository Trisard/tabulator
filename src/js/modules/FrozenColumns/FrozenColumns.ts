...
export default class FrozenColumns extends Module {
	...
	static moduleName: string = "frozenColumns";
	...
	initializeColumn(column: any): void {
		var config: any = {margin:0, edge:false};
	...
	layoutCalcRows(): void {
		if(this.table.modExists("columnCalcs")){
			const colCalcs = this.table.modules.columnCalcs as any;
			...
			if(this.table.modExists("groupRows")){
				this.layoutGroupCalcs((this.table.modules.groupRows as any).getGroups());
			}
		}
	}
	...
	reinitializeRows(): void {
		...
		var otherRows = this.table.rowManager.getRows().filter((row: any) => !visibleRows.includes(row));
	...
	layoutRow(row: any): void {
		if(this.table.options.layout === "fitDataFill" && this.rightColumns.length){
			(this.table.rowManager as any).getTableElement().style.minWidth = "calc(100% - " + this._calcSpace(this.rightColumns, this.rightColumns.length) + "px)";
		}
	...
	layoutElement(element: HTMLElement, column: any): void {
		...
		if(column.modules.frozen && element){
			...
			(element.style as any)[position] = column.modules.frozen.margin;
