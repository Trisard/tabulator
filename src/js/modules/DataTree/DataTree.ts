...
export default class DataTree extends Module{
	...
	static moduleName: string = "dataTree";
	...
	initialize(): void {
		if(this.table.options.dataTree){
			...
			switch(typeof options.dataTreeStartExpanded){
				case "boolean":
					this.startOpen = function(row: any, index: any){
				...
				default:
					this.startOpen = function(row: any, index: any){
						return (options.dataTreeStartExpanded as any)[index];
					};
					break;
			}
	...
	generateControlElement(row: any, el: HTMLElement): void {
		...
		if(config.children !== false){
			...
			if(config.open){
				...
				config.controlEl.addEventListener("click", (e: MouseEvent) => {
					e.stopPropagation();
			...
			config.controlEl.addEventListener("mousedown", (e: MouseEvent) => {
				e.stopPropagation();
			});
	...
	generateChildren(row: any): any[] {
		...
		childArray.forEach((childData: any) => {
	...
	rowDeleting(row: any): void {
		...
		if (config && config.children && Array.isArray(config.children)){
			config.children.forEach((childRow: any) => {
				if(childRow instanceof Row){
					childRow.wipe();
				}
			});
	...
	findChildIndex(subject: any, parent: any): any {
		...
		if(typeof subject == "object"){
			...
			if(subject instanceof Row){
				...
				match = (subject as any)._getSelf().data;
			}else if(typeof HTMLElement !== "undefined" && subject instanceof HTMLElement){
				if(parent.modules.dataTree){
					match = parent.modules.dataTree.children.find((childRow: any) => {
			...
			match = parent.data[this.field].find((row: any) => {
				return row.data[this.table.options.index as any] == subject;
			});
		}
