...
export default class SelectRange extends Module {
	...
	static moduleName: string = "selectRange";
	static moduleInitOrder: number = 1;
	static moduleExtensions: any = extensions;
	...
	initialize(): void {
		if (this.options("selectableRange")) {	
			...
			if(this.options('columns').findIndex((column: any) => column.frozen) > 0) {
				console.warn("Having frozen column in arbitrary position with selectRange option may result in unpredictable behavior.");
			}

			if(this.options('columns').filter((column: any) => column.frozen).length > 1) {
	...
	initializeTable(): void {		
		...
		this.table.rowManager.element.addEventListener("keydown", this.keyDownEvent as any);
	...
	updateHeaderColumn(): void {
		...
		if((this.table.modules as any).frozenColumns && (this.table.modules as any).frozenColumns.active){
			frozenCols = (this.table.modules as any).frozenColumns.getFrozenColumns();
	...
	_handleMouseUp(e: MouseEvent): void {
		this.mousedown = false;
		document.removeEventListener("mouseup", this.mouseUpEvent as any);
	}
	
	_handleKeyDown(e: KeyboardEvent): void {
		if (!this.blockKeydown && (!this.table.modules.edit || (this.table.modules.edit && !(this.table.modules.edit as any).currentCell))) {
			if (e.key === "Enter") {
				// is editing a cell?
				if (this.table.modules.edit && (this.table.modules.edit as any).currentCell) {
					return;
				}
				
				(this.table.modules.edit as any).editCell(this.getActiveCell());
	...
	handleColumnResized(column: any): void {
		...
		this.ranges.forEach((range) => {
			...
			selectedColumns.forEach((selectedColumn: any) => {
	...
	handleColumnMouseDown(event: MouseEvent, column: any): void {
		...
		document.addEventListener("mouseup", this.mouseUpEvent as any);
	...
	findJumpRow(column: any, rows: any[], reverse: boolean, emptyStart: boolean, emptySide: boolean): any {
		...
		return this.findJumpItem(emptyStart, emptySide, rows, function(row: any){return row.getData()[column.getField()];});
	}
	...
	findJumpCol(row: any, columns: any[], reverse: boolean, emptyStart: boolean, emptySide: boolean): any {
		...
		return this.findJumpItem(emptyStart, emptySide, columns, function(column: any){return row.getData()[column.getField()];});
	}
	...
	autoScroll(range: Range, row: HTMLElement, column: HTMLElement): void {
		...
		view = {
			...
			bottom:	tableHolder.scrollTop +	tableHolder.offsetHeight - (this.table.rowManager as any).scrollbarWidth,
		};
	...
	layoutElement(visibleRows?: boolean): void {
		...
		rows.forEach((row: any) => {
			if (row.type === "row") {
				this.layoutRow(row);
				row.cells.forEach((cell: any) => this.renderCell(cell));
			}
		});
	...
	getCell(rowIdx: number, colIdx: number): any {
		...
		return row ? row.getCells(false, true).filter((cell: any) => cell.column.visible)[colIdx] : null;
	}
	...
	resetRanges(): Range {
		...
		if(this.table.rowManager.activeRows.length){
			visibleCells = this.table.rowManager.activeRows[0].cells.filter((cell: any) => cell.column.visible);
	...
	tableDestroyed(): void {
		document.removeEventListener("mouseup", this.mouseUpEvent as any);
		if(this.overlay){
			this.table.rowManager.element.removeEventListener("keydown", this.keyDownEvent as any);
		}
	}
	
	selectedRows(component?: boolean): any[] {
		return component ? this.activeRange.getRows().map((row: any) => row.getComponent()) : this.activeRange.getRows();
	}
	
	selectedColumns(component?: boolean): any[] {
		return component ? this.activeRange.getColumns().map((col: any) => col.getComponent()) : this.activeRange.getColumns();
	}
