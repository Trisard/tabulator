...
export default class Persistence extends Module {
	...
	initialize(): void {
		if(this.table.options.persistence){
			...
			this.mode = (mode !== true ?  mode : (this.localStorageTest() ? "local" : "cookie")) as string;

			if(this.table.options.persistenceReaderFunc){
				if(typeof this.table.options.persistenceReaderFunc === "function"){
					this.readFunc = this.table.options.persistenceReaderFunc;
				}else{
					if((Persistence.readers as any)[this.table.options.persistenceReaderFunc]){
						this.readFunc = (Persistence.readers as any)[this.table.options.persistenceReaderFunc];
					}else{
						console.warn("Persistence Read Error - invalid reader set", this.table.options.persistenceReaderFunc);
					}
				}
			}else{
				if((Persistence.readers as any)[this.mode]){
					this.readFunc = (Persistence.readers as any)[this.mode];
				}else{
			...
			if(this.table.options.persistenceWriterFunc){
				if(typeof this.table.options.persistenceWriterFunc === "function"){
					...
					if((Persistence.writers as any)[this.table.options.persistenceWriterFunc]){
						this.writeFunc = (Persistence.writers as any)[this.table.options.persistenceWriterFunc];
					}else{
						console.warn("Persistence Write Error - invalid reader set", this.table.options.persistenceWriterFunc);
					}
				}
			}else{
				if((Persistence.writers as any)[this.mode]){
					this.writeFunc = (Persistence.writers as any)[this.mode];
				}else{
	...
	retrieveData(type: string){
		return this.readFunc ? this.readFunc(this.id, type) : false;
	}

	//merge old and new column definitions
	mergeDefinition(oldCols: any[] | undefined, newCols: any[], mergeAllNew?: boolean){
		var output: any[] = [];
	...
	_findColumn(columns: any[], subject: any){
