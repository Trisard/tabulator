...
export default function(this: any, list: any[], options: any, setFileContents: (data: any, mime: string) => void){
	...
	function generateSheet(){
		...
		merges: any[] = [],
		worksheet: any = {},
		range = {s: {c:0, r:0}, e: {c:(list[0] ? list[0].columns.reduce((a: number, b: any) => a + (b && b.width ? b.width : 1), 0) : 0), r:list.length }};

		//parse row list
		list.forEach((row, i) => {
			var rowData: any[] = [];

			row.columns.forEach(function(col: any, j: number){
