...
export default class VirtualDomVertical extends Renderer {
	...
	rerenderRows(callback?: () => void): void {
		...
		var topRow: number | false = false;
		var topOffset: number | false = false;
	...
	_virtualRenderFill(position?: number, forceMove?: boolean, offset?: number): void {
		...
		if(rowsCount && Helpers.elVisible(this.elementVertical)){
			...
			while(((rowsToRender == rowsCount || rowsHeight <= containerHeight + this.vDomWindowBuffer) || totalRowsRendered < this.vDomWindowMinTotalRows) && this.vDomBottom < rowsCount -1) {
				...
				const rowsNeedingHeightInit: any[] = [];
				renderedRows.forEach((row) => {
			...
			if(forceMove){
				this.scrollTop = this.vDomTopPad + (topPadHeight) + offset - (this.elementVertical.scrollWidth > this.elementVertical.clientWidth ? (this.elementVertical as HTMLElement).offsetHeight - containerHeight : 0);
			}
			...
			if(this.elementVertical.scrollWidth > this.elementVertical.clientWidth && forceMove){
				this.scrollTop += (this.elementVertical as HTMLElement).offsetHeight - containerHeight;
			}
			...
			if(this.vDomTop){
				...
				if(row && i < this.vDomMaxRenderChain){
					...
					if(fillableSpace >= rowHeight){
						...
						if(!initialized){
							rowHeight = (row.getElement() as HTMLElement).offsetHeight;
