...
class Helpers {
    ...
    static deepClone(obj, clone, list = []) {
        var objectProto = Object.getPrototypeOf({}), arrayProto = Object.getPrototypeOf([]);
        if (!clone) {
...
let Popup$1 = class Popup extends CoreFeature {
    constructor(table, element, parent = null) {
        super(table);
    ...
    elementPositionCoords(element, position = "right") {
        var offset = Helpers.elOffset(element), containerOffset, x = 0, y = 0;
        if (this.container !== document.body) {
...
class Module extends CoreFeature {
    ...
    popup(menuEl, menuContainer) {
        return new Popup$1(this.table, menuEl, menuContainer || null);
    }
    ///////////////////////////////////
    //////// Alert Management ////////
    ///////////////////////////////////
...
function generateParamsList$1(data, prefix) {
    ...
    if (Array.isArray(data)) {
        data.forEach((item, i) => {
            output = output.concat(generateParamsList$1(item, prefix ? prefix + "[" + i + "]" : i.toString()));
        });
    }
...
//load defaults
Ajax.defaultConfig = config;
Ajax.defaultURLGenerator = urlBuilder;
Ajax.defaultLoaderPromise = defaultLoaderPromise;
Ajax.contentTypeFormatters = contentTypeFormatters;

const pasteActions$1 = {
...
class Clipboard extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("clipboard", undefined);
        this.registerColumnOption("titleClipboard", undefined);
    }
    initialize() {
    ...
    copy(range, internal) {
        ...
        if (this.mode === true || this.mode === "copy") {
            ...
            if (typeof window.getSelection != "undefined" && typeof document.createRange != "undefined") {
                ...
                rangeObj.selectNodeContents(this.table.element);
                sel = window.getSelection();
                if (sel && sel.toString() && internal) {
    ...
    checkPasteOrigin(e) {
        ...
        if (blocked || (e.target && !["DIV", "SPAN"].includes(e.target.tagName))) {
            valid = false;
        }
        return valid;
    }
...
Clipboard.moduleExtensions = extensions$4;
//load defaults
Clipboard.pasteActions = pasteActions$1;
Clipboard.pasteParsers = pasteParsers$1;
...
class Column extends CoreFeature {
    ...
    setWidthActual(width) {
        ...
        if (isNaN(parsedWidth)) {
            parsedWidth = Math.floor((this.table.element.clientWidth / 100) * parseInt(width));
        }
...
Column.defaultOptionList = defaultOptions$1;
...
class DataTree extends Module {
    ...
    initialize() {
        if (this.table.options.dataTree) {
            ...
            this.indent = options.dataTreeChildIndent || 0;
...
function json$2 (list, options, setFileContents) {
    ...
    var output = JSON.stringify(fileContents, null, '\t');
    setFileContents(output, "application/json");
}
...
function xlsx$1 (list, options, setFileContents) {
    ...
    if (options.sheetOnly) {
        setFileContents(generateSheet(), "application/octet-stream");
        return;
    }
    if (options.sheets) {
...
function input (cell, onRendered, success, cancel, editorParams) {
    ...
    if (editorParams.elementAttributes && typeof editorParams.elementAttributes == "object") {
        for (let key in editorParams.elementAttributes) {
            if (key.charAt(0) == "+") {
                let attrKey = key.slice(1);
                input.setAttribute(attrKey, input.getAttribute(attrKey) + editorParams.elementAttributes[key]);
            }
            else {
...
function date$1 (cell, onRendered, success, cancel, editorParams) {
    var inputFormat = editorParams.format, vertNav = editorParams.verticalNavigation || "editor", DT = inputFormat ? this.table.dependencyRegistry.lookup(["luxon", "DateTime"], "DateTime") : null;
    //create and style input
    var cellValue = cell.getValue(), input = document.createElement("input");
    ...
    _createInputElement() {
        ...
        if (attribs && typeof attribs == "object") {
            for (let key in attribs) {
                if (key.charAt(0) == "+") {
                    let attrKey = key.slice(1);
                    input.setAttribute(attrKey, (input.getAttribute(attrKey) || "") + attribs[key]);
                }
                else {
    ...
    _defaultSortFunction(as, bs) {
        ...
        if (!as && as !== 0) {
            ...
            if (!(rd.test(a) && rd.test(b)))
                return a > b ? 1 : -1;
            let aMatch = a.match(rx);
            let bMatch = b.match(rx);
            if (aMatch && bMatch) {
                L = aMatch.length > bMatch.length ? bMatch.length : aMatch.length;
                while (i < L) {
                    a1 = aMatch[i];
                    b1 = bMatch[i++];
                    if (a1 !== b1) {
            ...
            if (!el) {
                ...
                if (item.elementAttributes && typeof item.elementAttributes == "object") {
                    for (let key in item.elementAttributes) {
                        if (key.charAt(0) == "+") {
                            let attrKey = key.slice(1);
                            el.setAttribute(attrKey, (this.input.getAttribute(attrKey) || "") + item.elementAttributes[key]);
                        }
                        else {
...
function star$1 (cell, onRendered, success, cancel, editorParams) {
        ...
        stars.forEach(function (star, i) {
            if (i < val) {
                if (self.table.browser == "ie") {
                    ...
                    star.classList.remove("tabulator-star-inactive");
                    star.classList.add("tabulator-star-active");
                }
                ...
                if (self.table.browser == "ie") {
                    ...
                    star.classList.remove("tabulator-star-active");
                    star.classList.add("tabulator-star-inactive");
                }
    ...
    if (editorParams.elementAttributes && typeof editorParams.elementAttributes == "object") {
        for (let key in editorParams.elementAttributes) {
            if (key.charAt(0) == "+") {
                let attrKey = key.slice(1);
                starsHolder.setAttribute(attrKey, (starsHolder.getAttribute(attrKey) || "") + editorParams.elementAttributes[key]);
            }
            else {
...
function progress$1 (cell, onRendered, success, cancel, editorParams) {
    ...
    if (editorParams.elementAttributes && typeof editorParams.elementAttributes == "object") {
        for (let key in editorParams.elementAttributes) {
            if (key.charAt(0) == "+") {
                let attrKey = key.slice(1);
                bar.setAttribute(attrKey, (bar.getAttribute(attrKey) || "") + editorParams.elementAttributes[key]);
            }
            else {
...
class Edit extends Module {
    constructor(table) {
        ...
        this.registerComponentFunction("cell", "navigatePrev", (cell) => this.navigatePrev(cell));
        this.registerComponentFunction("cell", "navigateNext", (cell) => this.navigateNext(cell));
        this.registerComponentFunction("cell", "navigateLeft", (cell) => this.navigateLeft(cell));
        this.registerComponentFunction("cell", "navigateRight", (cell) => this.navigateRight(cell));
        this.registerComponentFunction("cell", "navigateUp", (cell) => this.navigateUp(cell));
        this.registerComponentFunction("cell", "navigateDown", (cell) => this.navigateDown(cell));
    }
    initialize() {
        this.subscribe("cell-init", this.bindEditor.bind(this));
        this.subscribe("cell-delete", (cell) => this.clearEdited(cell));
        ...
        if (!this.confirm("edit-nav-disabled")) {
            this.subscribe("keybinding-nav-prev", (e) => this.navigatePrev(undefined, e));
            this.subscribe("keybinding-nav-next", this.keybindingNavigateNext.bind(this));
            this.subscribe("keybinding-nav-up", (e) => this.navigateUp(undefined, e));
            this.subscribe("keybinding-nav-down", (e) => this.navigateDown(undefined, e));
        }
    ...
    keybindingNavigateNext(e) {
        var cell = this.currentCell, newRow = this.table.options.tabEndNewRow;
        if (cell) {
...
class Export extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("htmlOutput", undefined);
        this.registerColumnOption("titleHtmlOutput", undefined);
    }
    initialize() {
...
Export.columnLookups = columnLookups$1;
Export.rowLookups = rowLookups$1;
...
const filters = {
    ...
    "keywords": function (filterVal, rowVal, rowData, filterParams) {
        var keywords = String(filterVal).toLowerCase().split(typeof filterParams.separator === "undefined" ? " " : filterParams.separator), value = String(rowVal === null || typeof rowVal === "undefined" ? "" : rowVal).toLowerCase(), matches = [];
...
function link (cell, formatterParams, onRendered) {
    ...
    function labelTraverse(path, data) {
        var item = path.shift(), value = data[item];
...
function toggle (cell, formatterParams, onRendered) {
    var value = cell.getValue(), size = formatterParams.size || 15, sizePx = size + "px", containEl, switchEl, onValue = formatterParams.hasOwnProperty("onValue") ? formatterParams.onValue : true, offValue = formatterParams.hasOwnProperty("offValue") ? formatterParams.offValue : false, state;
...
class Format extends Module {
    constructor(table) {
        super(table);
        this.registerColumnOption("formatter", undefined);
        this.registerColumnOption("formatterParams", undefined);
        this.registerColumnOption("formatterPrint", undefined);
        this.registerColumnOption("formatterPrintParams", undefined);
        this.registerColumnOption("formatterClipboard", undefined);
        this.registerColumnOption("formatterClipboardParams", undefined);
        this.registerColumnOption("formatterHtmlOutput", undefined);
        this.registerColumnOption("formatterHtmlOutputParams", undefined);
        this.registerColumnOption("titleFormatter", undefined);
        this.registerColumnOption("titleFormatterParams", undefined);
    }
    initialize() {
...
class FrozenColumns extends Module {
    constructor(table) {
        ...
        this.active = false;
        this.blocked = true;
        this.registerColumnOption("frozen", undefined);
    }
    //reset initial state
    reset() {
        this.initializationMode = "left";
    ...
    layoutCalcRows() {
        if (this.table.modExists("columnCalcs")) {
            const colCalcs = this.table.modules.columnCalcs;
            if (colCalcs.topInitialized && colCalcs.topRow) {
                this.layoutRow(colCalcs.topRow);
            }
            if (colCalcs.botInitialized && colCalcs.botRow) {
                this.layoutRow(colCalcs.botRow);
            }
    ...
    layoutRow(row) {
        if (this.table.options.layout === "fitDataFill" && this.rightColumns.length) {
            this.table.rowManager.getTableElement().style.minWidth = "calc(100% - " + this._calcSpace(this.rightColumns, this.rightColumns.length) + "px)";
        }
    ...
    configureGroupSetup() {
        if (this.table.options.groupBy) {
            ...
            if (startOpen) {
                ...
                this.startOpen = startOpen.map((level) => {
...
class History extends Module {
    ...
    undo() {
        if (this.index > -1) {
            ...
            console.warn(this.table.options.history ? "History Undo Error - No more history to undo" : "History module not enabled");
            return false;
        }
    }
    redo() {
        if (this.history.length - 1 > this.index) {
            ...
            console.warn(this.table.options.history ? "History Redo Error - No more history to redo" : "History module not enabled");
            return false;
        }
    }
...
History.moduleExtensions = extensions$3;
//load defaults
History.undoers = undoers;
History.redoers = redoers;
...
class HtmlTableImport extends Module {
    ...
    parseTable() {
        var element = this.table.originalElement, options = this.table.options, headers = element.getElementsByTagName("th"), rowsBody = element.getElementsByTagName("tbody")[0], data = [];
        this.hasIndex = false;
    ...
    pickFile(extensions, importReader) {
        return new Promise((resolve, reject) => {
            ...
            input.addEventListener("change", (e) => {
                ...
                var file = files[0], reader = new FileReader(), valid = this.validateFile(file);
                if (valid === true) {
...
class Interaction extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("headerClick", undefined);
        this.registerColumnOption("headerDblClick", undefined);
        this.registerColumnOption("headerContext", undefined);
        this.registerColumnOption("headerMouseEnter", undefined);
        this.registerColumnOption("headerMouseLeave", undefined);
        this.registerColumnOption("headerMouseOver", undefined);
        this.registerColumnOption("headerMouseOut", undefined);
        this.registerColumnOption("headerMouseMove", undefined);
        this.registerColumnOption("headerMouseDown", undefined);
        this.registerColumnOption("headerMouseUp", undefined);
        this.registerColumnOption("headerTap", undefined);
        this.registerColumnOption("headerDblTap", undefined);
        this.registerColumnOption("headerTapHold", undefined);
        this.registerColumnOption("cellClick", undefined);
        this.registerColumnOption("cellDblClick", undefined);
        this.registerColumnOption("cellContext", undefined);
        this.registerColumnOption("cellMouseEnter", undefined);
        this.registerColumnOption("cellMouseLeave", undefined);
        this.registerColumnOption("cellMouseOver", undefined);
        this.registerColumnOption("cellMouseOut", undefined);
        this.registerColumnOption("cellMouseMove", undefined);
        this.registerColumnOption("cellMouseDown", undefined);
        this.registerColumnOption("cellMouseUp", undefined);
        this.registerColumnOption("cellTap", undefined);
        this.registerColumnOption("cellDblTap", undefined);
        this.registerColumnOption("cellTapHold", undefined);
    }
    initialize() {
        ...
        symbols.forEach((symbol) => {
            switch (symbol) {
                ...
                case "meta":
                    ...
                    break;
                default:
                    let numSymbol = isNaN(symbol) ? symbol.toUpperCase().charCodeAt(0) : parseInt(symbol);
...
//load defaults
Keybindings.bindings = bindings$1;
Keybindings.actions = actions$1;
...
class Menu extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("headerContextMenu", undefined);
        this.registerColumnOption("headerClickMenu", undefined);
        this.registerColumnOption("headerDblClickMenu", undefined);
        this.registerColumnOption("headerMenu", undefined);
        this.registerColumnOption("headerMenuIcon", undefined);
        this.registerColumnOption("contextMenu", undefined);
        this.registerColumnOption("clickMenu", undefined);
        this.registerColumnOption("dblClickMenu", undefined);
    }
    initialize() {
        this.deprecatedOptionsCheck();
...
class MoveColumns extends Module {
    ...
    bindTouchEvents(column) {
        var colEl = column.getElement(), startXMove = false, //shifting center position of the cell
...
class MoveRows extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("rowHandle", undefined);
    }
    createPlaceholderElement() {
        var el = document.createElement("div");
        el.classList.add("tabulator-row");
...
//load defaults
MoveRows.senders = senders;
MoveRows.receivers = receivers;
...
class Page extends Module {
    ...
    initializePaginator(hidden) {
        ...
        if (!hidden) {
            ...
            this.count = this.table.options.paginationButtonCount || 5;
        }
        //set default values
        this.mode = this.table.options.paginationMode || "local";
    }
    ...
    setPageToRow(row) {
        ...
        var index = rows ? rows.indexOf(row) : -1;
        if (index > -1) {
    ...
    _parseRemoteData(data) {
        ...
        if (typeof data.last_page === "undefined") {
            console.warn("Remote Pagination Error - Server response missing '" + (this.table.options.dataReceiveParams?.last_page || "last_page") + "' property");
        }
        if (data.data) {
            ...
            if (this.progressiveLoad) {
                ...
                if (this.page > this.max) {
                    ...
                    paginationOutOfRange = this.table.options.paginationOutOfRange;
            ...
            console.warn("Remote Pagination Error - Server response missing '" + (this.table.options.dataReceiveParams?.data || "data") + "' property");
        }
        return data.data;
    }
...
//load defaults
Page.pageCounters = pageCounters;
...
//load defaults
Persistence.readers = readers;
Persistence.writers = writers;
...
class ReactiveData extends Module {
    ...
    watchData(data) {
        ...
        Object.defineProperty(this.data, "splice", {
            ...
            configurable: true,
            value: function (...args) {
                var start = args[0] < 0 ? data.length + args[0] : args[0], end = args[1], newRows = args[2] ? args.slice(2) : false, startRow, result;
                if (!self.blocked && version === self.currentVersion) {
    ...
    rebuildTree(row) {
        const treeMod = this.table.modules.dataTree;
...
class ResizeColumns extends Module {
    ...
    columnWidthUpdated(column) {
        if (column.modules.frozen) {
            const frozenMod = this.table.modules.frozenColumns;
            if (frozenMod.leftColumns.includes(column)) {
                frozenMod.leftColumns.forEach((col) => {
                    this.reinitializeColumn(col);
                });
            }
            else if (frozenMod.rightColumns.includes(column)) {
                frozenMod.rightColumns.forEach((col) => {
                    this.reinitializeColumn(col);
                });
            }
        }
...
ResponsiveLayout.moduleExtensions = extensions$2;
...
class SelectRow extends Module {
    ...
    handleComplexRowClick(row, e) {
        if (e.shiftKey) {
            ...
            if (lastClickedRowIdx !== false && rowIdx !== false) {
                ...
                if (e.ctrlKey || e.metaKey) {
                    toggledRows.forEach((toggledRow) => {
                        if (toggledRow !== this.lastClickedRow) {
                            if (this.table.options.selectableRows !== true && !this.isRowSelected(row)) {
                                if (typeof this.table.options.selectableRows === "number" && this.selectedRows.length < this.table.options.selectableRows) {
                    ...
                    if (this.table.options.selectableRows !== true) {
                        if (typeof this.table.options.selectableRows === "number" && toggledRows.length > this.table.options.selectableRows) {
    ...
    _deselectRow(rowInfo, silent, force) {
...
SelectRow.moduleExtensions = extensions$1;
...
const pasteActions = {
    range: function (data) {
        var rows = [], range = this.table.modules.selectRange.activeRange, singleCell = false, bounds, startCell, startRow, rowWidth, dataLength;
...
class SelectRange extends Module {
    constructor(table) {
        ...
        this.activeRangeCellElement = null;
        this.rowHeader = null;
    ...
    initialize() {
        if (this.options("selectableRange")) {
            ...
            if (this.options('columns').filter((column) => column.frozen).length > 1) {
    ...
    initializeWatchers() {
        ...
        this.subscribe("columns-loaded", () => this.updateHeaderColumn());
    ...
    layoutRanges() {
        ...
        if (this.activeRangeCellElement) {
            if (this.table.rtl) {
    ...
    addRange(start, end) {
        ...
        if (this.rangeContainer && range.element) {
...
SelectRange.moduleInitOrder = 1;
SelectRange.moduleExtensions = extensions;
...
class Sort extends Module {
    constructor(table) {
        ...
        this.registerColumnOption("sorter", undefined);
        this.registerColumnOption("sorterParams", undefined);
        this.registerColumnOption("headerSort", true);
        this.registerColumnOption("headerSortStartingDir", undefined);
        this.registerColumnOption("headerSortTristate", undefined);
    }
    initialize() {
        this.subscribe("column-layout", this.initializeColumn.bind(this));
        this.subscribe("table-built", this.tableBuilt.bind(this));
    ...
    initializeColumn(column) {
        ...
        if (column.definition.headerSort !== false) {
            ...
            switch (typeof this.table.options.headerSortElement) {
                case "function":
                    //do nothing
                    break;
                case "object":
                ...
                switch (typeof value) {
                    ...
                    default:
                        if (!isNaN(value) && value !== "") {
                            ...
                            if (String(value).match(/((^[0-9]+[a-z]+)|(^[a-z]+[0-9]+))+$/i)) {
                                sorter = "alphanum";
                            }
                        }
                        break;
...
class ColumnManager extends CoreFeature {
    constructor(table) {
        ...
        this.scrollLeft = 0;
        this.optionsList = new OptionsList(this.table, "column definition", defaultOptions$1);
    ...
    initializeRenderer() {
        ...
        if (renderClass) {
            this.renderer = new renderClass(this.table, this.element, this.table.tableElement);
    ...
    _addColumn(definition, before, nextToColumn) {
        var column = new Column(definition, this, false), colEl = column.getElement(), index = nextToColumn ? this.findColumnIndex(nextToColumn) : -1;
...
class RowManager extends CoreFeature {
    ...
    initializePlaceholder() {
        ...
        if (placeholder) {
            ...
            if (typeof placeholder == "string") {
                ...
                // Placeholder invalid, handled by default null initialization
            }
            this.placeholder = el;
        }
    }
    ...
    findRow(subject) {
        if (typeof subject == "object") {
            ...
            //subject should be treated as the index of the row
            let match = this.rows.find((row) => {
                return row.data[this.table.options.index || "id"] == subject;
            });
            return match || false;
        }
        //catch all for any other type of input
    ...
    moveRowInArray(rows, from, to, after) {
        var fromIndex, toIndex = -1, start, end = -1;
        if (from !== to) {
    ...
    refreshPipelines(handler, stage, index, renderInPosition) {
        ...
        if (stage === "all" || stage === "dataPipeline") {
    ...
    setActiveRows(activeRows) {
        this.activeRows = Object.assign([], activeRows);
    ...
    _positionPlaceholder() {
        if (this.placeholder && this.placeholder.parentNode && this.placeholderContents) {
            this.placeholder.style.width = this.table.columnManager.getWidth() + "px";
            this.placeholderContents.style.width = this.element.clientWidth + "px";
...
class InteractionMonitor extends CoreFeature {
    constructor(table) {
        super(table);
        this.el = null;
    ...
    updateEventListeners() {
        for (let key in this.listeners) {
            ...
            if (listener.components.length) {
                if (!listener.handler) {
                    listener.handler = (e) => this.track(key, e);
    ...
    load(data, params, config, replace, silent, columnsChanged) {
        ...
        if (this.confirm("data-loading", [data, params, config, silent])) {
            ...
            return result.then((response) => {
                ...
                setTimeout(() => {
                    this.clearAlert();
                }, this.table.options.dataLoaderErrorTimeout || 0);
            })
                .finally(() => {
                this.loading = false;
            });
...
class InternalEventBus {
    ...
    _debugConfirm(...args) {
        ...
        return this._confirm.apply(this, args);
    }
}
...
class Localize extends Module {
    ...
    initialize() {
        ...
        const langsOptions = this.table.options.langs;
...
ModuleBinder.moduleBindings = {};
ModuleBinder.moduleExtensions = {};
ModuleBinder.modulesRegistered = false;
ModuleBinder.defaultModules = false;
...
class Tabulator extends ModuleBinder {
    ...
    /////////////// Event Functions  ///////////////
    on(event, callback) {
        this.externalEvents.subscribe(event, callback);
    }
    off(event) {
